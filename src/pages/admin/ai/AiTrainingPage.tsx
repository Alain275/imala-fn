...
const STATUS_STYLE: Record<TrainingJob['status'], { badge: string; icon: React.ElementType; label: string }> = {
  queued: { badge: 'bg-muted text-muted-foreground border-border', icon: Clock, label: 'Queued' },
  running: { badge: 'bg-sky-500/10 text-sky-600 dark:text-sky-400 border-sky-200 dark:border-sky-800/40', icon: Loader2, label: 'Running' },
  completed: { badge: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800/40', icon: CheckCircle2, label: 'Completed' },
  failed: { badge: 'bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-200 dark:border-rose-800/40', icon: AlertCircle, label: 'Failed' },
}
...
export default function AiTrainingPage() {
  ...
  const handleStartJob = async () => {
    ...
    try {
      ...
      setStartOpen(false)
      toast.success(`Training job "${job.name}" queued`)
    } catch {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      setJobs(prev => prev.map(j => j.id === cancelTarget.id ? { ...j, status: 'failed' as const, errorMessage: 'Cancelled by admin.' } : j))
      toast.success(`Job "${cancelTarget.name}" cancelled`)
  ...
  const handleRetry = async (job: TrainingJob) => {
    ...
    try {
      ...
      toast.success(`Retry queued: "${newJob.name}"`)
    } catch {
  ...
  return (
    <div className="min-h-screen bg-background">
      <Header title="Training Jobs" subtitle="Start, monitor, and manage model training runs" />
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-muted-foreground">
              {loading ? '—' : `${jobs.length} total jobs · ${jobs.filter(j => j.status === 'completed').length} completed`}
            </p>
          </div>
          <Button className="gap-2" onClick={() => setStartOpen(true)}>
            <PlayCircle className="w-4 h-4" /> Start Training Run
          </Button>
        </div>
        ...
        {loading ? (
          ...
          <Card className="border-0 shadow-md">
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <p className="font-semibold text-foreground">No training jobs yet</p>
              <p className="text-sm text-muted-foreground mt-1">Start a training run to begin.</p>
            </CardContent>
          </Card>
        ) : (
          <div className="space-y-4">
                  ...
                  <CardContent className="p-5">
                    <div className="flex items-start gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground mb-3">
                          <span>Base: <span className="text-foreground font-medium">{job.baseModel}</span></span>
                          <span>Epochs: <span className="text-foreground font-medium">{job.epochs}</span></span>
                          <span>Dataset: <span className="text-foreground font-medium">v{job.datasetVersion}</span></span>
                          {lastMetric && (
                            <span>Best acc: <span className="text-emerald-600 dark:text-emerald-400 font-medium">{(lastMetric.valAccuracy * 100).toFixed(1)}%</span></span>
                          )}
                        </div>
                        {job.status === 'running' && (
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {job.metrics.length > 0 && (
                          ...
                          >
                            Metrics {isExpanded ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                          </Button>
                        )}
                        <DropdownMenu>
                    ...
                    {isExpanded && job.metrics.length > 0 && (
                      <div className="mt-4 pt-4 border-t border-border">
                        <p className="text-xs font-semibold text-muted-foreground mb-3">Training Metrics (Loss & Accuracy per Epoch)</p>
                        <div className="h-48">
                          <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={job.metrics}>
                              <CartesianGrid strokeDasharray="3 3" vertical={false} className="stroke-muted" />
                              <XAxis dataKey="epoch" className="fill-muted-foreground" fontSize={10} tickLine={false} label={{ value: 'Epoch', position: 'insideBottom', offset: -2, className: 'fill-muted-foreground', fontSize: 10 }} />
                              ...
                              <Line yAxisId="right" type="monotone" dataKey="loss" stroke="#f59e0b" strokeWidth={1.5} dot={false} name="Train Loss" />
                              <Line yAxisId="right" type="monotone" dataKey="valLoss" stroke="#ef4444" strokeWidth={1.5} dot={false} strokeDasharray="4 2" name="Val Loss" />
                              <Line yAxisId="left" type="monotone" dataKey="accuracy" stroke="#10b981" strokeWidth={1.5} dot={false} name="Train Acc" />
                              <Line yAxisId="left" type="monotone" dataKey="valAccuracy" stroke="#0ea5e9" strokeWidth={1.5} dot={false} strokeDasharray="4 2" name="Val Acc" />
                            </LineChart>
                          </ResponsiveContainer>
                        </div>
                        <div className="flex items-center gap-5 mt-2 flex-wrap">
                          {[['#10b981', 'Train Acc'], ['#0ea5e9', 'Val Acc'], ['#f59e0b', 'Train Loss'], ['#ef4444', 'Val Loss']].map(([c, l]) => (
      ...
      <AlertDialog open={!!cancelTarget} onOpenChange={open => !open && setCancelTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will immediately stop the training run. Progress will be lost.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
